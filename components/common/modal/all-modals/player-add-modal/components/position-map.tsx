...
export default function SelectPosition({
  ...
  return (
    <div className="mx-auto w-full max-w-4xl rounded-2xl bg-[#090B10] p-4 text-white">
      <ModalStepHeader
        title={"Position Map"}
        subtitle={"Select the player's primary and secondary positions. Select the positions on the field bellow  "}
        ...
        <div className="mt-6 space-y-6">
          <div className="rounded-xl">
            <div className="relative w-full overflow-hidden rounded-md border border-white/20 bg-white/5">
              ...
              /> */}
              <PositionMap data={selectedPositionArray} />
            </div>
          </div>
          ...
          <div className="flex gap-4 "> 
            ...
            >
              ...
              <SelectContent position="popper" className=" ">
                <SelectGroup>
                  {Number(positions?.length) > 0 && positions?.map((position , idx)=> ( 
                    <SelectItem key={idx} value={String(position.id)} className="hover:bg-brand!  "  >{position.name}</SelectItem>
                  ))} 
                </SelectGroup>
              </SelectContent>
            </Select>
