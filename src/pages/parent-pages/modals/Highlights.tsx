...
export default function Highlights() {
  ...
  const goBackToBiography = () => {
    setModalStep("biography")
  }
  ...
  const goToNextStep = () => {
    const hasUploadedReel = uploadedItems.some((item) => item.type === "video")
    const hasAddedLink = uploadedItems.some((item) => item.type === "link")

    if (!hasUploadedReel && !hasAddedLink) {
      setSubmitError(
        "Please upload at least one highlight reel or add at least one external link before continuing."
      )
      return
    }
...
click to browse. Max size 500MB."
      ...
      <StepActions onBack={goBackToBiography} onNext={goToNextStep} />
    </div>
  )
}
