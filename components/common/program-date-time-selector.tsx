...
export default function ProgramDateTimeSelector({
  ...
  const [monthData, setMonthData] = useState<any[]>([])
  // Init with the month actually shown by the calendar (startDate's month, if any)
  // so the very first fetch matches what's on screen.
  ...
  // Fetch available dates for the month — runs on mount (first render) and
  // whenever the user changes month in the calendar.
  useEffect(() => {
    const getMonth = async () => {
      try {
