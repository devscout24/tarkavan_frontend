...
import { setUnreadCount } from "@/lib/features/userSlice"
...
export default function MessagePage() {
  ...
  const [activeConvId, setActiveConvId] = React.useState<string>(
    chatHeads.find((chat) => String(chat.receiver_id) === receiver_chatId)
      ?.conversation_id || ""
  )
  ...
  return (
    <section className="flex h-[90dvh] min-h-0 flex-col p-3">
      <div className="flex min-h-0 flex-1 flex-col gap-4 lg:flex-row">
        ...
        <ChatInbox
          receiver_chatId={receiver_chatId || ""}
          messages={messages}
          chatHead={
            chatHeads.find(
              (chat) => String(chat.receiver_id) === receiver_chatId
            ) as TChatHeadItem
          }
        />
      </div>
