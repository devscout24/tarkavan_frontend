...
/** Pulls `res.data.data` out of the app's standard `{ success, data: { data } }` API shape. */
...
/** Reads and JSON-parses a value stored in localStorage (e.g. the "go_elite_user" object). */
...
/** Reads a plain string value from localStorage (e.g. the "go_elite_token"), no JSON parsing. */
...
export default function ProgramDateTimeSelector({
  ...
  // Single source of truth for "which date is currently active" —
  // drives both the calendar UI and the available-times fetch.
  // Seeded with startDate so the first render already fetches slots for it.
  const [date, setDate] = useState<Date | undefined>(startDate)
  const [user, setUser] = useState<{ role?: string } | null>(null)

  const [monthData, setMonthData] = useState<any[]>([])
  const [currentMonth, setCurrentMonth] = useState(
    moment(startDate ?? undefined).format("YYYY-MM")
  )
  ...
  const currentDateKey = date ? moment(date).format("YYYY-MM-DD") : ""
  ...
  const availableDatesFromAPI = useMemo(
    () =>
      monthData.filter((d: any) => d.has_slots).map((d: any) => parseISO(d.date)),
    [monthData]
  )
  ...
  useEffect(() => {
    setUser(readLocalStorageJSON("go_elite_user"))
  }, [])

  // Fetch available dates for the month — runs on mount and whenever the
  // user changes month in the calendar.
  useEffect(() => {
    const getMonth = async () => {
      try {
        const res = await getDateForMonth({ program_id: programid, month: currentMonth })
        const data = unwrap<{ days: any[] }>(res)
        if (data) setMonthData(data.days)
      } catch (err) {
  ...
  // Fetch available times whenever `date` changes — including on first
  // render, since `date` is seeded with programStartDate.
  useEffect(() => {
    if (!date) return
    const fetchAvailableTimes = async () => {
      try {
        ...
        const data = unwrap<{ times: TTimeSlot[] }>(res)
        if (data) setAvailableTimes(data.times)
      } catch (err) {
  ...
  // Child list for a parent booking on behalf of a child
  useEffect(() => {
    const getChild = async () => {
      try {
        const res = await getChildList()
        const data = unwrap<TChield[]>(res)
        if (data) setAllChields(data)
      } catch (err) {
  ...
  // { program_id, athlete_profile_id, slots: [{ booking_date, booking_time_id }] }

  const buildPayload = (athleteProfileId: string | number) => ({
    program_id: Number(detailsID),
    athlete_profile_id: Number(athleteProfileId),
    slots: Object.entries(selectedSlots).flatMap(([booking_date, ids]) =>
      ids.map((booking_time_id) => ({ booking_date, booking_time_id }))
    ),
  })
  ...
  const handleBooking = async (bookBy: "parent" | "player") => {
    const token = readLocalStorageString("go_elite_token")
    const currentUser = readLocalStorageJSON<{ profile_id?: string | number }>("go_elite_user")
    ...
    const athleteId =
      bookBy === "parent" ? selectedChildId || child_id : String(currentUser?.profile_id)
    ...
    try {
      ...
      const data = unwrap<{ checkout_url: string | null }>(res)
      if (data) {
        if (data.checkout_url) {
          window.location.href = data.checkout_url
        } else if (data.checkout_url === null && (res as any)?.data?.status) {
          toast.success((res as any)?.data?.message || "Free Booking successful.")
        }
      }
    } catch (err) {
  ...
  return (
    <div className="mt-4 space-y-6 rounded-2xl bg-white p-4 sm:p-6">
      ...
      <Calendar
        ...
        onMonthChange={(month) => setCurrentMonth(moment(month).format("YYYY-MM"))}
      />
          ...
          <div className="flex items-center justify-between">
            ...
            {currentDateIds.length > 0 && (
              <button
                type="button"
                onClick={() =>
                  setSelectedSlots((prev) => ({ ...prev, [currentDateKey]: [] }))
                }
              ...
              return (
                <Button
                  ...
                  className={`relative h-10 rounded-xl bg-white text-sm font-medium text-[#202020] hover:bg-[#F8F8F8] ${isDisabled ? "line-through opacity-50" : ""} ${
                    isSelected ? "border-2 border-brand bg-brand/5" : "border border-[#DEDEDE]"
                  } `}
      ...
      {selectionSummary && (
        <div className="flex items-center justify-between rounded-xl border border-brand/30 bg-brand/5 px-3 py-2">
          <p className="text-sm font-medium text-[#191919]">{selectionSummary}</p>
          <button
            type="button"
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-t border-[#DEDEDE] pt-4">
        <p className="text-lg font-medium text-primary!">Total: $ {priceToShow}</p>
