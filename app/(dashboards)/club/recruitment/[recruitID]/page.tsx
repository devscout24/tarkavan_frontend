...
export default function Page() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('recruit_details_loaded', handleRecruitDetailsLoaded)

    return () => {
      window.removeEventListener('recruit_details_loaded', handleRecruitDetailsLoaded)
    }
  ...
  const handleChangeStatus = async (applicantId: string, status: string) => { 
    ...
    try{
      ...
      const res = await changeStatus({ applicantId: applicantId as string, status: formData }) 
      console.log(res)
      if(res && 'success' in res && res.success && res.data && 'data' in res.data && res.data.data){
          ...
          <TableBody>
            {data.map((data) => (
              ...
              >
                ...
                >
                  <Image
                    src={data?.profile_image || "https://github.com/shadcn.png"}
                    alt="https://github.com/shadcn.png"
                    width={100}
                    height={100}
                    className="max-h-12.5 max-w-12.5 min-w-12.5 min-h-12.5 rounded-full "
                  />
                  <div className="">
                    <h3 className="text-base font-medium">{data.name}</h3>
                    <p className="mt-1 text-brand">{data.recruitment.coach_position}</p>
                  </div>
                </TableCell>
                <TableCell className={columnBorderClass}>{data.recruitment.team_name}</TableCell>
                ...
                <TableCell className={`${columnBorderClass}`}>
                  <div className="flex items-center justify-center gap-5 text-white">
                    <FiEye className="text-lg text-brand    " onClick={()=> router.push(`/profile/${data.user_id}`) } />
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent>
                        <DropdownMenuGroup> 
                          <DropdownMenuItem onClick={() => handleChangeStatus(String(data.application_id), "accepted")} className="hover:bg-brand!">Accept</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleChangeStatus(String(data.application_id), "scheduled")} className="hover:bg-brand!">Scheduled</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleChangeStatus(String(data.application_id), "rejected")} className="hover:bg-brand!">Reject</DropdownMenuItem>
                        </DropdownMenuGroup> 
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
