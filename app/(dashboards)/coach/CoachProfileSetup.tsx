...
export default function CoachProfileSetup({ 
  ...
  const [formData, setFormData] = useState<CoachProfileFormData>(getInitialFormData)
  ...
  const updateFormData = useCallback((updates: Partial<CoachProfileFormData>) => {
    setFormData(prev => {
  ...
  // Validation function
  const validateForm = (data: CoachProfileFormData): { isValid: boolean; errors: string[] } => {
    const errors: string[] = []
  ...
  const handleSubmit = useCallback(async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const result: CoachProfileApiResult = await createOrUpdateCoachProfile(apiFormData)
  ...
  const handleBasicInfoUpdate = useCallback((data: any) => {
    updateFormData({
      name: data.firstName || '',
  ...
  const handleSportsUpdate = useCallback((data: any) => {
    updateFormData({
      sports: data.sport || '',
