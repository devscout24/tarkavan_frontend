...
const SOCIAL_LINKS = [
  ...
  },
  {
    ...
    placeholder: "https://www.whatsapp.com/yourprofile",
  },
]
...
const dividerCls =
  "mt-3 h-px w-full bg-[repeating-linear-gradient(90deg,rgba(255,255,255,0.12)_0,rgba(255,255,255,0.12)_12px,transparent_12px,transparent_22px)]"
...
export default function CoachProfileSetup({
  ...
  const [formData, setFormData] =
    useState<CoachProfileFormData>(INITIAL_FORM_DATA)
    ...
    const fetchOptions = async () => {
      try {
        ...
        if (
          ...
          setFormattedRoleOptions(
            positionsResult.data.map((p: any) => ({
              value: p.id.toString(),
              label: p.name,
            }))
          )
  ...
  useEffect(() => {
    ...
    const resolveRole = (): string => {
      ...
      if (typeof r === "string") {
        return (
          roleOptions.find((o) => o.id.toString() === r)?.id.toString() ??
          roleOptions.find((o) => o.name === r)?.id.toString() ??
          r
        )
      }
    ...
    const gender: "male" | "female" | "other" = validGenders.includes(
      editData.gender as any
    )
    ...
    setFormData((prev) => ({
      ...
      // stripe_country: editData.stripe_country || "",
      // stripe_holder_name: editData.stripe_holder_name || "",
      // stripe_account_email: editData.stripe_account_email || "",
    }))
  ...
  const addTitle = () => {
    ...
    if (!exists)
      setField("coaching_title", [
        ...(formData.coaching_title || []),
        normalized,
      ])
    setTitleInput("")
  }
  ...
  const removeTitle = (title: string) =>
    setField(
      "coaching_title",
      formData.coaching_title?.filter((t) => t !== title) || []
    )
  ...
  return (
    <section className="bg-primary">
      <div className="space-y-4 rounded-[16px] bg-primary p-4 sm:p-6">
        {/* Header */}
        <div className="space-y-4 pb-3 text-white">
        ...
        <div className="space-y-5 rounded-[16px] bg-secondary/20 p-4">
          {/* Photo Upload */}
          <div>
            ...
            <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
              {[
                ...
                <div key={key} className="space-y-2">
                  ...
                  <ShadInput
                    ...
                    onChange={(e) => {
                      setField(
                        key as keyof CoachProfileFormData,
                        e.target.value
                      )
                      updateBasicInfo?.({
                        ...formData,
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    const norm = (
                      ["male", "female", "other"].includes(v) ? v : "male"
                    ) as "male" | "female" | "other"
                    setField("gender", norm)
                    updateBasicInfo?.({ ...formData, gender: norm })
                  }}
                >
                  <SelectTrigger className={`${selectTriggerCls} py-5!`}>
          ...
          <div className="mt-5 space-y-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setField("sports", v)}
              >
                <SelectTrigger className={`${selectTriggerCls} py-5!`}>
            ...
            <div className="space-y-2">
              ...
              <Select
                value={formData.current_role as string}
                onValueChange={(v) => setField("current_role", v)}
              >
                <SelectTrigger className={`${selectTriggerCls} py-5!`}>
          ...
          <div className="mt-5 space-y-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setField("years_of_experience", v)}
              >
                <SelectTrigger className={`${selectTriggerCls} py-5!`}>
              ...
              >
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const updatedMeta = credFiles.filter(
                      (f) => f.id !== file.id
                    )
                    const updatedFiles = credFileObjects.filter(
                      (_, i) => i !== idx
                    )
                    setCredFiles(updatedMeta)
                    setCredFileObjects(updatedFiles)
        ...
        <section className={sectionCls}>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-5">
            <CheckboxRow
              label="Player-centric approach"
              checked={formData.player_centric_approach}
              onToggle={() =>
                setField(
            ...
            />
            <CheckboxRow
              label="Data-driven training"
              checked={formData.data_driving_training}
              onToggle={() =>
                setField(
