import { useRouter } from "next/navigation"
import { Icon } from "./Icon"

interface AddChildCardProps {
  onGetStarted?: () => void
}

const AddChildrenIcon = () => (
  <Icon width="28" height="20" viewBox="0 0 28 20">
    <path
      d="M21.25 12.5V8.75H17.5V6.25H21.25V2.5H23.75V6.25H27.5V8.75H23.75V12.5H21.25ZM10 10C8.625 10 7.44792 9.51042 6.46875 8.53125C5.48958 7.55208 5 6.375 5 5C5 3.625 5.48958 2.44792 6.46875 1.46875C7.44792 0.489583 8.625 0 10 0C11.375 0 12.5521 0.489583 13.5312 1.46875C14.5104 2.44792 15 3.625 15 5C15 6.375 14.5104 7.55208 13.5312 8.53125C12.5521 9.51042 11.375 10 10 10ZM0 20V16.5C0 15.7917 0.182292 15.1406 0.546875 14.5469C0.911458 13.9531 1.39583 13.5 2 13.1875C3.29167 12.5417 4.60417 12.0573 5.9375 11.7344C7.27083 11.4115 8.625 11.25 10 11.25C11.375 11.25 12.7292 11.4115 14.0625 11.7344C15.3958 12.0573 16.7083 12.5417 18 13.1875C18.6042 13.5 19.0885 13.9531 19.4531 14.5469C19.8177 15.1406 20 15.7917 20 16.5V20H0ZM2.5 17.5H17.5V16.5C17.5 16.2708 17.4427 16.0625 17.3281 15.875C17.2135 15.6875 17.0625 15.5417 16.875 15.4375C15.75 14.875 14.6146 14.4531 13.4688 14.1719C12.3229 13.8906 11.1667 13.75 10 13.75C8.83333 13.75 7.67708 13.8906 6.53125 14.1719C5.38542 14.4531 4.25 14.875 3.125 15.4375C2.9375 15.5417 2.78646 15.6875 2.67188 15.875C2.55729 16.0625 2.5 16.2708 2.5 16.5V17.5ZM10 7.5C10.6875 7.5 11.276 7.25521 11.7656 6.76562C12.2552 6.27604 12.5 5.6875 12.5 5C12.5 4.3125 12.2552 3.72396 11.7656 3.23438C11.276 2.74479 10.6875 2.5 10 2.5C9.3125 2.5 8.72396 2.74479 8.23438 3.23438C7.74479 3.72396 7.5 4.3125 7.5 5C7.5 5.6875 7.74479 6.27604 8.23438 6.76562C8.72396 7.25521 9.3125 7.5 10 7.5Z"
      fill="white"
    />
  </Icon>
)

const ArrowIcon = () => (
  <Icon width="20" height="20" viewBox="0 0 20 20">
    <path
      d="M10.834 15.0016L15.834 10.0015L10.834 5.00159M15.4173 10.0016H4.16734"
      stroke="#C6F57A"
      strokeWidth="1.5"
      strokeLinecap="square"
    />
  </Icon>
)



export default function AddChildCard({ onGetStarted }: AddChildCardProps) {
  
  const router = useRouter()

  return (
    <div onClick={()=> router.push("?add-new=player")} className="flex max-h-113 max-w-87.25 flex-col items-center justify-center gap-4 rounded-[24px] border border-dashed border-secondary/50 p-6 lg:max-w-75 lg:gap-3 lg:p-5 xl:max-w-87.25 xl:gap-4 xl:p-6">
      <div 
        className="flex h-14 w-14 cursor-pointer items-center justify-center rounded-full bg-secondary/30 lg:h-12 lg:w-12 xl:h-14 xl:w-14"
      >
        <AddChildrenIcon />
      </div>

      <div className="space-y-2 text-center">
        <p className="text-lg leading-[120%] font-medium text-white lg:text-base xl:text-lg">
          Add Your Children
        </p>
        <p className="text-sm leading-[150%] font-normal text-secondary lg:text-xs xl:text-sm">
          Add a profile for your other children to manage their soccer career.
        </p>
      </div>

      <button
        onClick={onGetStarted}
        className="flex cursor-pointer items-center gap-1 text-sm font-medium text-brand lg:text-xs xl:text-sm"
      >
        Get Started
        <ArrowIcon />
      </button>
    </div>
  )
}
