...
const DEFAULT_ACCEPT = "video/mp4,video/quicktime,image/*"

export default function UploadReel({
  className = "",
  title = "Upload Highlight Reel",
  subtitle = "Drag and drop your MP4, MOV files here or click to browse. Max size 500MB.",
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (maxFileSizeBytes) {
      ...
      if (oversizedFile) {
        onValidationError?.(
          maxFileSizeMessage ??
            `Each file must be ${(maxFileSizeBytes / (1024 * 1024)).toFixed(0)}MB or less.`
        )
        event.target.value = ""
        return
      }
  ...
  return (
    <div
      className={`rounded-xl border border-dashed border-white/20 bg-secondary/40 px-4 py-6 sm:px-6 ${className}`}
    >
      <input
        ref={fileInputRef}
        type="file"
      ...
      <button
        ...
        className="mx-auto flex w-full max-w-59.75 cursor-pointer flex-col items-center gap-3 rounded-xl border-0 p-4 text-center outline-none focus:outline-none focus-visible:ring-0 focus-visible:outline-none"
      >
        <span className="inline-flex h-14 w-14 items-center justify-center rounded-full bg-secondary/60">
          <Icon width="28" height="20" viewBox="0 0 28 20" aria-hidden="true">
            <path
        ...
        <div className="space-y-1">
          <p className="text-center text-[14px] leading-[107.143%] font-medium text-white uppercase">
            {title}
          </p>
          <p className="text-center text-[12px] leading-[150%] font-normal text-white/70">
            {subtitle}
          </p>
        </div>
      </button>
